        ...
        private void btnThanhToan_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                using (var context = new Model1())
                {
                    ...
                    // Kiểm tra nếu đã có dữ liệu trùng trong bảng DoanhThu
                    var doanhThuExists = context.DoanhThus.Any(dt => dt.idLichChieu == lblidlc.Text && dt.idPhim == phim.id);
                    if (doanhThuExists)
                    {
                        MessageBox.Show("Doanh thu cho lịch chiếu và phim này đã được ghi nhận!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
